...
overlapping_areas = map(lambda c: overlapping_area(claim_to_rectangle(c[0]), claim_to_rectangle(c[1])), overlapping_claims) 

#print(list(overlapping_areas))
all_overlapping_areas = functools.reduce(lambda a,b: a|b, overlapping_areas)
      
#print(claim_to_rectangle(claims[0]))
#print(all_overlapping_areas)
print(len(all_overlapping_areas))
