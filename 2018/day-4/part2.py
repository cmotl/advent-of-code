...
def most_asleep_minutes(schedule):
  ...
  for m,count in minutes.items():
    if count > most_minutes:
      ...
      most_minutes = count
  return (most_frequent_minute, most_minutes)


sleepy_minutes = {k: most_asleep_minutes(v) for k,v in schedules_by_guard[0].items()} 


def sleepiest_guard_and_minutes(sleepy_minutes):
  most_asleep_times = 0
  most_asleep_guard = None
  most_asleep_minute = 0
    
  for g,minutes in sleepy_minutes.items():
    print(minutes)
    if minutes[1] > most_asleep_times:
      most_asleep_times = minutes[1]
      most_asleep_minute = minutes[0]
      most_asleep_guard = g

  return (most_asleep_guard, most_asleep_minute, most_asleep_times)

g = sleepiest_guard_and_minutes(sleepy_minutes)

print(schedules_by_guard[0][179])
